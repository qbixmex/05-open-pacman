// render.js
// Arcade drawing on canvas. Uses game.grid (not MAZE) to reflect eaten dots.
...
// Arcade-style walls: thin rounded lines connecting the centers of adjacent
// wall cells. Produces the original's continuous outline.
function drawWalls( ctx, grid ) {
  ...
  for ( let y = 0; y < H; y++ ) {
    for ( let x = 0; x < W; x++ ) {
      ...
      // Connecting only right and down avoids duplicate strokes.
      ...
      // Isolated wall cell (no neighbor): short dash so it's visible.
      const lone =
        ( x + 1 >= W || grid[ y ][ x + 1 ] !== 1 ) &&
        ( x - 1 < 0 || grid[ y ][ x - 1 ] !== 1 ) &&
        ( y + 1 >= H || grid[ y + 1 ][ x ] !== 1 ) &&
...
function drawPacman( ctx, p, frame ) {
  ...
  // Animated mouth: opens/closes with the frame.
...
function drawGhost( ctx, g, color ) {
  ...
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.arc( cx, cy - 1, r, Math.PI, 0, false ); // head
  ...
  // eyes looking according to direction
...
function drawHUD( ctx, game, W ) {
  ...
  ctx.textAlign = 'right';
  ctx.fillText( 'LIVES ' + game.lives, W * TILE - 8, 4 );
}
